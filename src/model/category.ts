...
export enum ErrorCode {
  ...
  TAKEN_CATEGORY_NAME = "TAKEN_CATEGORY_NAME",
}
...
export async function get_id(id: number): Promise<CategoryResponse> {
  try {
    const exist_category = await db("categories")
      .select("*")
      .where({ id })
...
export async function delete_(name: string): Promise<CategoryResponse> {
  try {
    const exist_category = await db("categories")
      .select("*")
      .where({
